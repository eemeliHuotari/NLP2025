...
STOP = {
    ...
    "be","been","with","without","from","that","this","these","those","as","it","its","into"
}
def toks(s: str):
...
def synset_to_Mlabel(syn, cuis_for_term, cui2gloss):
    ...
    if syn is None:
        return "M1"
    syn_text = " ".join([syn.definition()] + syn.examples() + [" ".join(syn.lemma_names())])
...
def run_maxsim(term, sent, measure, ic=None):
    try:
        return max_similarity(term, sent, measure, pos='n', ic=ic)
    except Exception:
        return None
...
def main():
    ...
    ap.add_argument("--cui_gloss_json", required=True, help="JSON mapping {CUI: 'definition text ...'}")
    ap.add_argument("--subset_10_10", action="store_true", help="Evaluate on a random 10 acronyms + 10 regular terms")
    ...
    for term in term2cuis:
        path = (idx.get(_normalize_name(term))
    ...
    if args.subset_10_10:
        k_acr = min(10, len(acr_terms)); k_reg = min(10, len(reg_terms))
    ...
    def algo_predictors():
        ...
        algos.append(wrap("MaxSim wup",    lambda term, text: run_maxsim(term, text, 'wup')))
        algos.append(wrap("MaxSim lch",    lambda term, text: run_maxsim(term, text, 'lch')))
        algos.append(wrap("MaxSim path",   lambda term, text: run_maxsim(term, text, 'path')))
        if brown_ic is not None:
            algos.append(wrap("MaxSim res",  lambda term, text: run_maxsim(term, text, 'res', ic=brown_ic)))
            algos.append(wrap("MaxSim jcn",  lambda term, text: run_maxsim(term, text, 'jcn', ic=brown_ic)))
            algos.append(wrap("MaxSim lin",  lambda term, text: run_maxsim(term, text, 'lin', ic=brown_ic)))
        def run_max_lemma_count(term, sent):
            try:
                from pywsd.baseline import max_lemma_count as _mlc
                try:
                    return _mlc(term, pos='n')
                except TypeError:
                    return _mlc(term)
            except ValueError:
                pass
    ...
    if missing_map:
        ...
        print(f"[cov] Wrote template with {len(missing_map)} missing CUIs → {tmpl_path}")
