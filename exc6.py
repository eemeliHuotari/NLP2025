...
def _simple_soundex(word: str) -> str:
    ...
    if not w: return ""
    first = w[0]
    codes = {**dict.fromkeys(list("BFPV"), "1"),
             **dict.fromkeys(list("CGJKQSXZ"), "2"),
             **dict.fromkeys(list("DT"), "3"),
             "L":"4", **dict.fromkeys(list("MN"), "5"), "R":"6"}
    digits = [codes.get(ch, "") for ch in w[1:]]
    out = []; prev = codes.get(first, "")
    for d in digits:
        if d != prev: out.append(d)
        if d != "": prev = d
    sdx = first + "".join(out)
...
print("Simple Lesk + hyper/hyponyms:", synset_str(simple_lesk(sent, target, pos='n', hyperhypo=True)))

try:
    print("Cosine Lesk                 :", synset_str(cosine_lesk(sent, target, pos='n')))
except TypeError:
...
def try_pyswd_ic(measure):
    try:
        ...
        try:
            return max_similarity(target, sent, measure, pos='n', ic=wordnet_ic.ic('ic-brown.dat'))
        except TypeError:
            try:
                return max_similarity(target, sent, measure, pos='n', IC=wordnet_ic.ic('ic-brown.dat'))
            except Exception:
...
for measure in ['res', 'jcn', 'lin']:
    ...
    if val is not None:
        ...
        score, best_target = ic_similarity_fallback(target, sent, which=measure, target_pos='n')
        print(f"MaxSim ({measure})            :", score, "(fallback via NLTK)")
        if best_target:
            print(f"  ↳ best target synset       : {best_target.name()} :: {best_target.definition()}")
...
print("\n== WUP + Phonetic (α = 0.5) ==")
(best_sc, best_wup, best_phn, best_syn), scored = wup_phonetic_best_synset(target, sent, alpha=0.5)
print(f"Best synset                  : {synset_str(best_syn)}")
print(f"Combined score               : {best_sc:.3f}  (wup={best_wup:.3f}, phonetic={best_phn:.3f})")

for sc, wup, phn, ss in scored[1:4]:
    print(f"  runner-up                  : {synset_str(ss)}")
    print(f"    scores                   : combined={sc:.3f}, wup={wup:.3f}, phonetic={phn:.3f}")
